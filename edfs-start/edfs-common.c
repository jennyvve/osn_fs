 ...
 */
static bool edfs_read_super(edfs_image_t *img) {
    ...
    if (buf.st_size < edfs_get_size(&img->sb)) {
        fprintf(stderr,
                "%s: %s: file system size larger than image size.\n",
                progname, img->filename);
        return false;
    }
...
/* Open an EdFS image file at @filename. If @read_super is true, the
 * super block is read into memory. Returns the image.
 */
edfs_image_t *edfs_image_open(const char *filename, bool read_super) {
...
/* Read @inode from disk; @inode->inumber indicates which inode to
 * read. Returns the amount of bytes read.
 */
int edfs_read_inode(edfs_image_t *img, edfs_inode_t *inode) {
    if (inode->inumber >= img->sb.inode_table_n_inodes)
        return -ENOENT;

    off_t offset = edfs_get_inode_offset(&img->sb, inode->inumber);
    return pread(img->fd, &inode->inode, sizeof(edfs_disk_inode_t),
                 offset);
}
 ...
 */
int edfs_read_root_inode(edfs_image_t *img, edfs_inode_t *inode) {
    inode->inumber = img->sb.root_inumber;
    return edfs_read_inode(img, inode);
}

/* Writes @inode to disk. @inode->inumber indicates which inode to
 * write to. Returns the amount of bytes written.
 */
int edfs_write_inode(edfs_image_t *img, edfs_inode_t *inode) {
    if (inode->inumber >= img->sb.inode_table_n_inodes)
        return -ENOENT;

    off_t offset = edfs_get_inode_offset(&img->sb, inode->inumber);
    return pwrite(img->fd, &inode->inode, sizeof(edfs_disk_inode_t),
                  offset);
}

/* Clears an inode on disk. @inode->inumber indicates which inode to
 * clear. Returns the amount of bytes written.
 */
int edfs_clear_inode(edfs_image_t *img, edfs_inode_t *inode) {
    if (inode->inumber >= img->sb.inode_table_n_inodes)
        return -ENOENT;
    ...
    return pwrite(img->fd, &disk_inode, sizeof(edfs_disk_inode_t),
                  offset);
}

/* Finds an unused (free) inode. NOTE: This does not yet allocate the
 * inode! The inode is allocated only after a valid inode has been
 * written to it. Returns the inumber of the free inode, or 0 if the
 * inode table is full.
 */
edfs_inumber_t edfs_find_free_inode(edfs_image_t *img) {
    edfs_inode_t inode = {.inumber = 1};

    for (; inode.inumber < img->sb.inode_table_n_inodes;
         inode.inumber++) {
 ...
 */
int edfs_new_inode(edfs_image_t *img, edfs_inode_t *inode,
                   edfs_inode_type_t type) {
    edfs_inumber_t inumber;

    inumber = edfs_find_free_inode(img);
    if (inumber == 0) return -ENOSPC;
